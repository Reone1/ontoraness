...
자세한 옵션은 [CLI 레퍼런스 →](packages/cli/docs/cli-reference.md)
...
│           └── minimal/
│
├── packages/cli/docs/              ← 사람을 위한 문서 (패키지에 포함)
│
...
> `.ontoraness/docs/` 는 AI를 위한 컨텍스트이고, `packages/cli/docs/` 는 사람을 위한 문서입니다.
...
자세한 스키마: [온톨로지 스키마 레퍼런스 →](packages/cli/docs/ontology-schema.md)
...
자세한 설정: [서브 에이전트 가이드 →](packages/cli/docs/sub-agents.md)
